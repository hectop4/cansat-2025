...
int first_height_limit = 300;
int top_height=400;
bool top=false
int second_height_limit=200;
...
void loop()
{
  ...
  String messageT= "T:"+String(temperature);
  String messageP= "P:"+String(pressure);
  String messageH= "H:"+String(altitude);
  String messageM= "M:"+String(ppm);

  if(altitude>first_height_limit && top == false){
    top = true;
  }
  ...
  enviarMensaje(messageT);
  delay(interval);
  enviarMensaje(messageP);
  delay(interval);
  enviarMensaje(messageH);
  delay(interval);
  enviarMensaje(messageM);
  delay(interval);
...
void enviarMensaje(String firstmessage){
